    ...
    .get(async (req, res) => {
      ...
      try {
        ...
        const issues = await Issue.aggregate([
          ...
          _id === undefined ? { $match: {} } : { $match: { _id: issueIdToSearch } },
          issue_title === undefined ? { $match: {} } : { $match: { title: issue_title } },
          issue_text === undefined ? { $match: {} } : { $match: { text: issue_text } },
          open === undefined ? { $match: {} } : { $match: { open: open.toLowerCase() === 'true' } },
          created_by === undefined ? { $match: {} } : { $match: { created_by } },
          assigned_to === undefined ? { $match: {} } : { $match: { assigned_to } },
          status_text === undefined ? { $match: {} } : { $match: { status: status_text } }
        ]);
